export class EmbeddedTextEditor {
  ...
  _getEditingBox(element, zoom, line) {
    ...
    // A little hack here; we don't specify the height, so the DirectEditing.js will always call provider.update(), even if the content doesn't have.
    return {
  ...
  update(element, newText, oldText) {
    const index = element.embeddedText.findIndex((t) => t.content === oldText);
    this._modeling.updateEmbeddedText(element, index, newText);
  }
}
