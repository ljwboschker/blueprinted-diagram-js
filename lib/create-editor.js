...
export function createDiagramEditor(container, blueprint) {
  ...
  const editor = {
    ...
    onItemChange(callback) {
      eventBus.on(['shape.changed', 'connection.changed'], event => callback(createItem(event.element)));
    },
  ...
  eventBus.on('shape.create', event => { console.log(event); });
  return editor;
}

function createItem(element) {
  // Create a stripped object without internal properties and functions
  const { children, labels, svg, viewBox, blueprint, ...stripped } = element;
  const strippedElement = deepClone(stripped);

  const getLabelContent = (key) => {
    return element.labels?.find(label => label.key === key)?.content;
  };
