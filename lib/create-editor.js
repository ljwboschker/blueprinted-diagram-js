...
export function createDiagramEditor(container, blueprint) {
  ...
  const editor = {
    ...
    refresh(shapeId) {
      ...
      eventBus.fire('label.refresh', { element: shape });
    },
    onItemChange(callback) {
      eventBus.on(['element.changed', 'connection.changed'], (event) => callback(createDiagramEvent(event.element)));
    },
    onItemRemove(callback) {
      eventBus.on(['element.removed', 'connection.removed'], (event) => callback(createDiagramEvent(event.element)));
    },
    onItemSelect(callback) {
      eventBus.on('element.click', (event) => callback(createDiagramEvent(event.element)));
    },
    ...
    onContextAction(callback) {
      eventBus.on('contextaction.click', (event) => callback(event.action, createDiagramEvent(event.element)));
    }
  };
