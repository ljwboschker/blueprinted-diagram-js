...
export function createDiagramEditor(container, blueprint) {
    ...
    var root = elementFactory.createRoot({ type: 'root' });
    ...
    const editor = {
        ...
        onItemChange(callback) {
            eventBus.on('item.changed', event => callback(createItem(event.element)));
        },
        onItemRemove(callback) {
            eventBus.on('item.removed', event => callback(event.element.id));
        },
        onItemSelect(callback) {
            eventBus.on('element.click', event => callback(createItem(event.element)));
        },
        onItemDoubleClick(callback) {
            eventBus.on('element.dblclick', event => callback(createItem(event.element)));
        },
    ...
    eventBus.on(['shape.changed', 'connection.changed' ], event => eventBus.fire('item.changed', eventBus.createEvent({ element: event.element })));
    eventBus.on('connection.changed', event => {
        eventBus.fire('item.changed', eventBus.createEvent({ element: event.element.source }));
        eventBus.fire('item.changed', eventBus.createEvent({ element: event.element.target }));
    });
    eventBus.on(['shape.removed', 'connection.removed' ], event => eventBus.fire('item.removed', eventBus.createEvent({ element: event.element })));
    eventBus.on('connection.removed', event => {
        // TODO: source and target still contain the connection :( Need another solution.
        eventBus.fire('item.changed', eventBus.createEvent({ element: event.element.source }));
        eventBus.fire('item.changed', eventBus.createEvent({ element: event.element.target }));
    });
