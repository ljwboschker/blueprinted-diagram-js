...
export function createDiagramEditor(container, blueprint) {
  ...
  const editor = {
    ...
    onItemRemove(callback) {
      eventBus.on(['shape.removed', 'connection.removed'], (event) => callback(createItem(event.element)));
    },
    ...
    onContextAction(callback) {
      eventBus.on('contextaction.click', (event) => callback(event.action, createItem(event.element)));
    }
  };
