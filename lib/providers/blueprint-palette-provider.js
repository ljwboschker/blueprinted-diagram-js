...
BlueprintPaletteProvider.prototype.getPaletteEntries = function() {
  ...
  function createPaletteItem(blueprint) {
    return {
      group: 'elements',
      title: blueprint.title,
      className: `palette-${blueprint.type}`,
      action: {
        click: function(event) {
          const shape = createShape(elementFactory, blueprint);
  ...
  const elements = blueprint.elements.reduce((all, blueprint) => {
    const key = `create-${blueprint.type}`;
    all[key] = createPaletteItem(blueprint);
    return all;
  }, {});
