...
import { calculatePositionInShape, parseAlign } from './utils/label-alignment-util';
...
export default function LabelBehavior(eventBus, blueprint, modeling, elementFactory) {
  ...
  this.postExecute(['shape.create'], (event) => {
    ...
    blueprintShape.labels.forEach((definition) => {
      ...
      const position = calculatePositionInShape(shape, label, definition.position);
      modeling.createLabel(shape, position, label);
    });
  });
  ...
  this.postExecute(['connection.create'], (event) => {
    ...
    const label = createLabel(content, data, { ...textOptions });
  ...
  function createLabel(content, data, textOptions) {
    const dimension = text.getDimensions(content, textOptions);
