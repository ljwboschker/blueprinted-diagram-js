...
export default function LabelBehavior(eventBus, modeling, elementFactory) {
    ...
    this.postExecute(['connection.create'], event => {
        ...
        const connectionRule = source.rules?.connect?.find(c => c.to === target.type);
        
        const label = createLabel(connectionRule.text, { ...connectionRule.textOptions.style }, { ...connectionRule.textOptions.box } );
