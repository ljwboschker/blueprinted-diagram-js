...
export default function LabelBehavior(eventBus, modeling, elementFactory) {
    ...
    this.postExecute(['shape.create'], event => {
        ...
        const blueprint = event.context.shape.blueprint;
        if (!blueprint.shape.labels) return;

        blueprint.shape.labels.forEach(definition => {
            const label = createLabel(definition.dataKey, definition.text, { ...definition.style }, { width: shape.width - 20 } );
            const labelPosition = {
    ...
    this.postExecute(['connection.create'], event => {
        ...
        const label = createLabel(rule.label.dataKey, rule.label.text, { ...rule.label.textOptions.style }, { ...rule.label.textOptions.box } );
        const labelCenter = getConnectionMidpoint(connection.waypoints);

        modeling.createLabel(connection, labelCenter, label);
    });

    function createLabel(dataKey, content, style, box) {
        const textOptions = {
            align: 'center-top',
            fitBox: true,
            style,
    ...
    eventBus.on('element.changed', event => {
        ...
        const label = event.element;
        if (!label.labelTarget || !label.dataKey) return;
        
        // Update the label targets data object
        label.labelTarget.data[label.dataKey] = label.content;
    });
}
