...
export default function LabelBehavior(eventBus, modeling, elementFactory) {
    ...
    this.postExecute(['shape.create'], event => {
        ...
        blueprint.shape.labels.forEach(definition => {
            const label = createLabel(definition.name, definition.text, { ...definition.style }, { width: shape.width - 20 } );
            const labelPosition = {
    ...
    this.postExecute(['connection.create'], event => {
        ...
        const label = createLabel('name', rule.text, { ...rule.textOptions.style }, { ...rule.textOptions.box } );
        const labelCenter = getConnectionMidpoint(connection.waypoints);

        modeling.createLabel(connection, labelCenter, label);
    });

    function createLabel(name, content, style, box) {
        const textOptions = {
            align: 'center-top',
            fitBox: true,
            style,
    ...
    this.postExecute('shape.update', event => {
        debugger;
    });
}
