...
export default function LabelBehavior(eventBus, blueprint, modeling, elementFactory) {
    ...
    this.postExecute(['shape.create'], event => {
        ...
        const blueprintElement = blueprint.elements.find(e => e.type === shape.type);

        if (!blueprintElement.shape.labels) return;

        blueprintElement.shape.labels.forEach(definition => {
            const label = createLabel(definition.key, definition.text, { ...definition.style }, { width: shape.width - 20 } );
            const labelPosition = {
