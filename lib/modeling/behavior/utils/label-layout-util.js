...
import {
  ...
  rotateVector,
  getAngle
} from './geometric-utils';

import { getAttachment } from './line-attachment-util';
...
export function findNewLabelLineStartIndex(oldWaypoints, newWaypoints, attachment, hints) {
  ...
  if (hints.segmentMove) {
    ...
    const { newSegmentStartIndex } = hints.segmentMove;
  ...
  if (hints.bendpointMove) {
    const { insert, bendpointIndex } = hints.bendpointMove;
    let newIndex;
...
export function getVisibleLabelAdjustment(event) {
  ...
  const newWaypoints = event.context.newWaypoints || event.context.connection.waypoints;
  const { oldWaypoints } = event.context;
